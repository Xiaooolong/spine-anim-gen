...
const ffmpegPrimary = process.env.FFMPEG_PATH || 'ffmpeg';
const ffmpegFallback = ffmpegStatic && ffmpegStatic !== ffmpegPrimary ? ffmpegStatic : null;
...
export const reencodeH264 = async (inputPath: string, logPath?: string) => {
  return new Promise<void>((res) => {
    ...
    const run = (exe: string, allowFallback: boolean) => {
      ...
      ff.on('error', async (err) => {
        if (allowFallback && ffmpegFallback && exe !== ffmpegFallback) {
          run(ffmpegFallback, false);
          return;
        }
        try {
          if (logPath) {
            const log = `FFMPEG SPAWN ERROR:\n${String(err)}\n\nEXE:\n${exe}\n\nARGS:\n${args.join(' ')}`;
    ...
    run(ffmpegPrimary, true);
  });
};

export const changeVideoSpeed = async (inputPath: string, speed: number, logPath?: string) => {
  if (speed === 1.0) return; // No change
  
  return new Promise<void>((res, rej) => {
    const outPath = inputPath.replace(/\.mp4$/i, '.speed.mp4');
    const setpts = `${(1/speed).toFixed(4)}*PTS`;
    const args = ['-y', '-i', inputPath, '-filter:v', `setpts=${setpts}`, '-an', outPath];
    ...
    const run = (exe: string, allowFallback: boolean) => {
      ...
      ff.on('error', async (err) => {
        ...
        try {
          if (logPath) {
            const log = `FFMPEG SPAWN ERROR:\n${String(err)}\n\nEXE:\n${exe}\n\nARGS:\n${args.join(' ')}`;
            await writeFile(logPath.replace(/\.log$/i, '.speed_err.log'), log);
          }
        } catch {}
        rej(err);
      });
      ff.on('close', async (code) => {
        if (code === 0) {
          ...
          if (logPath) await writeFile(logPath.replace(/\.log$/i, '.speed_err.log'), `EXE:\n${exe}\n\nARGS:\n${args.join(' ')}\n\nFFMPEG ERR:\n${e}`);
          rej(new Error(`Speed adjustment failed: ${e}`));
        }
      });
    };
...
`;
  ...
  const pathSep = isWin ? ';' : ':';
  const directPyExe = process.env.PYTHON_EXECUTABLE || (isWin ? 'python' : 'python3');
  const condaExe = process.env.CONDA_EXE || 'conda';
  const condaEnv = process.env.CONDA_ENV || (isWin ? 'openai' : '');
  ...
  return new Promise((resolve, reject) => {
    const spawnOpts = {
      ...
      env: {
        ...process.env,
        PYTHONPATH: process.env.PYTHONPATH ? `${process.env.PYTHONPATH}${pathSep}.` : '.',
      },
    };
    ...
    const attach = (py: ReturnType<typeof spawn>) => {
    ...
    const spawnDirect = () => attach(spawn(directPyExe, [tmpPy, videoPath, outOverlayPath, outJsonPath], spawnOpts));

    if (isWin && condaEnv) {
      const py = attach(spawn(condaExe, ['run', '-n', condaEnv, 'python', tmpPy, videoPath, outOverlayPath, outJsonPath], spawnOpts));
      py.on('error', () => {
        if (resolved) return;
        spawnDirect();
      });
