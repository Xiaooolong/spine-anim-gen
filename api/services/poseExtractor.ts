...
export const changeVideoSpeed = async (inputPath: string, speed: number, logPath?: string) => {
  ...
  return new Promise<void>((res, rej) => {
    ...
    const exe = ffmpegPath || 'ffmpeg';
    // setpts = (1/speed) * PTS. 2x speed -> 0.5*PTS. 0.5x speed -> 2.0*PTS.
    const setpts = `${(1/speed).toFixed(4)}*PTS`;
    // Drop audio (-an) to avoid sync issues or complex filtering
    ...
    ff.on('close', async (code) => {
      if (code === 0) {
        ...
        const mv = spawn('bash', ['-lc', `mv "${outPath}" "${inputPath}"`]);
        mv.on('close', () => {
          // If successful, also update the overlay generation command to respect the new video duration?
          // Actually, extractPose2D uses `videoPath` which is now the speed-adjusted video.
          // However, the `fps` reading in python script will read the NEW fps or the NEW duration.
          // If we used setpts, the duration changes but fps usually stays same, frame count changes.
          // Wait, if we slow down (0.5x), duration doubles.
          // The issue is that `pose_overlay.mp4` is generated by OpenCV VideoWriter.
          // In the python script:
          // fps = cap.get(cv2.CAP_PROP_FRAME_WIDTH) -> NO, CAP_PROP_FPS.
          // writer = cv2.VideoWriter(..., fps, ...)
          // If the input video (after speed change) has correct metadata, the overlay should be correct speed.
          // Let's verify if ffmpeg updates the duration metadata correctly. It should.
          res();
        });
      } else {
        if (logPath) await writeFile(logPath.replace(/\.log$/i, '.speed_err.log'), e);
